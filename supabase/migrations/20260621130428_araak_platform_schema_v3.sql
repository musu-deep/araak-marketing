/*
# منصة أراك - مخطط قاعدة البيانات الأساسي (نسخة 3 - مفصولة)

الجداول: team_members, roles, departments, permissions, role_permissions,
...
- is_platform_admin() يحدد المدير (رئيس تنفيذي/نائب/مسؤول تسويق)
- الأعضاء يرون ما أُسنِد إليهم فقط
*/
...
CREATE INDEX IF NOT EXISTS idx_opp_assignments_member ON public.opportunity_assignments(team_member_id);
...
CREATE INDEX IF NOT EXISTS idx_tenders_lead ON public.tenders(assigned_lead);

CREATE TABLE IF NOT EXISTS public.tender_stages (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE INDEX IF NOT EXISTS idx_checklists_tender ON public.tender_checklists(tender_id);
...
CREATE INDEX IF NOT EXISTS idx_tasks_tender ON public.tasks(tender_id);
...
CREATE INDEX IF NOT EXISTS idx_documents_tender ON public.documents(tender_id);
...
CREATE INDEX IF NOT EXISTS idx_alerts_target ON public.accountability_alerts(target_member);
CREATE INDEX IF NOT EXISTS idx_alerts_unread ON public.accountability_alerts(is_read);
...
CREATE TABLE IF NOT EXISTS public.ai_advisor_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tender_id uuid REFERENCES public.tenders(id) ON DELETE CASCADE,
  question text,
  analysis jsonb,
...
ALTER TABLE public.tender_stages ENABLE ROW LEVEL SECURITY;
...
-- سياسات القراءة العامة للبيانات المرجعية
...
CREATE POLICY "opp_select_admin_or_assigned" ON public.opportunities FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR created_by = public.current_member_id()
    OR suggested_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.opportunity_assignments oa WHERE oa.opportunity_id = opportunities.id AND oa.team_member_id = public.current_member_id())
  );
DROP POLICY IF EXISTS "opp_insert_authenticated" ON public.opportunities;
CREATE POLICY "opp_insert_authenticated" ON public.opportunities FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "opp_update_admin_or_owner" ON public.opportunities FOR UPDATE TO authenticated
  ...
  WITH CHECK (true);
DROP POLICY IF EXISTS "opp_delete_admin" ON public.opportunities;
CREATE POLICY "opp_delete_admin" ON public.opportunities FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "opp_assign_select_admin_or_self" ON public.opportunity_assignments;
CREATE POLICY "opp_assign_select_admin_or_self" ON public.opportunity_assignments FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR team_member_id = public.current_member_id()
    OR assigned_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.opportunities o WHERE o.id = opportunity_assignments.opportunity_id AND (o.created_by = public.current_member_id() OR public.is_platform_admin()))
  );
DROP POLICY IF EXISTS "opp_assign_insert_admin_or_creator" ON public.opportunity_assignments;
CREATE POLICY "opp_assign_insert_admin_or_creator" ON public.opportunity_assignments FOR INSERT TO authenticated
  WITH CHECK (
    public.is_platform_admin()
    OR assigned_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.opportunities o WHERE o.id = opportunity_id AND o.created_by = public.current_member_id())
  );
...
CREATE POLICY "opp_assign_update_admin_or_creator" ON public.opportunity_assignments FOR UPDATE TO authenticated
  ...
  WITH CHECK (true);
DROP POLICY IF EXISTS "opp_assign_delete_admin" ON public.opportunity_assignments;
CREATE POLICY "opp_assign_delete_admin" ON public.opportunity_assignments FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "tender_select_admin_or_assigned" ON public.tenders;
CREATE POLICY "tender_select_admin_or_assigned" ON public.tenders FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR assigned_lead = public.current_member_id()
    OR created_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.tasks t WHERE t.tender_id = tenders.id AND t.assigned_to = public.current_member_id())
    OR EXISTS (SELECT 1 FROM public.tender_stages ts WHERE ts.tender_id = tenders.id AND ts.owner_id = public.current_member_id())
  );
DROP POLICY IF EXISTS "tender_insert_authenticated" ON public.tenders;
CREATE POLICY "tender_insert_authenticated" ON public.tenders FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "tender_update_admin_or_assigned" ON public.tenders FOR UPDATE TO authenticated
  ...
  WITH CHECK (true);
DROP POLICY IF EXISTS "tender_delete_admin" ON public.tenders;
CREATE POLICY "tender_delete_admin" ON public.tenders FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "stage_select_admin_or_assigned" ON public.tender_stages;
CREATE POLICY "stage_select_admin_or_assigned" ON public.tender_stages FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR owner_id = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.tenders t WHERE t.id = tender_stages.tender_id AND (t.assigned_lead = public.current_member_id() OR t.created_by = public.current_member_id()))
  );
DROP POLICY IF EXISTS "stage_insert_admin_or_lead" ON public.tender_stages;
CREATE POLICY "stage_insert_admin_or_lead" ON public.tender_stages FOR INSERT TO authenticated
  WITH CHECK (
    public.is_platform_admin()
    OR EXISTS (SELECT 1 FROM public.tenders t WHERE t.id = tender_id AND (t.assigned_lead = public.current_member_id() OR t.created_by = public.current_member_id()))
  );
...
CREATE POLICY "stage_update_admin_or_owner" ON public.tender_stages FOR UPDATE TO authenticated
  ...
  WITH CHECK (true);
DROP POLICY IF EXISTS "stage_delete_admin" ON public.tender_stages;
CREATE POLICY "stage_delete_admin" ON public.tender_stages FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "checklist_select_admin_or_related" ON public.tender_checklists;
CREATE POLICY "checklist_select_admin_or_related" ON public.tender_checklists FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR checked_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.tenders t WHERE t.id = tender_checklists.tender_id AND (t.assigned_lead = public.current_member_id() OR t.created_by = public.current_member_id()))
  );
DROP POLICY IF EXISTS "checklist_insert_admin_or_lead" ON public.tender_checklists;
CREATE POLICY "checklist_insert_admin_or_lead" ON public.tender_checklists FOR INSERT TO authenticated
  WITH CHECK (
    public.is_platform_admin()
    OR EXISTS (SELECT 1 FROM public.tenders t WHERE t.id = tender_id AND (t.assigned_lead = public.current_member_id() OR t.created_by = public.current_member_id()))
  );
...
CREATE POLICY "checklist_update_admin_or_related" ON public.tender_checklists FOR UPDATE TO authenticated
  USING (
    public.is_platform_admin()
    OR checked_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.tenders t WHERE t.id = tender_checklists.tender_id AND t.assigned_lead = public.current_member_id())
  )
  WITH CHECK (true);
DROP POLICY IF EXISTS "checklist_delete_admin" ON public.tender_checklists;
CREATE POLICY "checklist_delete_admin" ON public.tender_checklists FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "task_select_admin_or_assigned" ON public.tasks;
CREATE POLICY "task_select_admin_or_assigned" ON public.tasks FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
...
CREATE POLICY "task_insert_authenticated" ON public.tasks FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "task_update_admin_or_assigned" ON public.tasks FOR UPDATE TO authenticated
  ...
  WITH CHECK (true);
DROP POLICY IF EXISTS "task_delete_admin_or_creator" ON public.tasks;
CREATE POLICY "task_delete_admin_or_creator" ON public.tasks FOR DELETE TO authenticated
  USING (public.is_platform_admin() OR assigned_by = public.current_member_id());

DROP POLICY IF EXISTS "doc_select_admin_or_related" ON public.documents;
CREATE POLICY "doc_select_admin_or_related" ON public.documents FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR uploaded_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.tasks t WHERE t.tender_id = documents.tender_id AND t.assigned_to = public.current_member_id())
    OR EXISTS (SELECT 1 FROM public.tenders td WHERE td.id = documents.tender_id AND (td.assigned_lead = public.current_member_id() OR td.created_by = public.current_member_id()))
    OR EXISTS (SELECT 1 FROM public.opportunities o WHERE o.id = documents.opportunity_id AND (o.created_by = public.current_member_id() OR o.suggested_by = public.current_member_id()))
  );
DROP POLICY IF EXISTS "doc_insert_authenticated" ON public.documents;
CREATE POLICY "doc_insert_authenticated" ON public.documents FOR INSERT TO authenticated WITH CHECK (true);
...
CREATE POLICY "doc_delete_admin" ON public.documents FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "docver_select_admin_or_related" ON public.document_versions;
CREATE POLICY "docver_select_admin_or_related" ON public.document_versions FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR uploaded_by = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.documents d WHERE d.id = document_versions.document_id AND (d.uploaded_by = public.current_member_id() OR public.is_platform_admin()))
  );
...
DROP POLICY IF EXISTS "alert_select_admin_or_target" ON public.accountability_alerts;
CREATE POLICY "alert_select_admin_or_target" ON public.accountability_alerts FOR SELECT TO authenticated
...
CREATE POLICY "alert_insert_auth" ON public.accountability_alerts FOR INSERT TO authenticated WITH CHECK (true);
...
DROP POLICY IF EXISTS "alert_delete_admin" ON public.accountability_alerts;
CREATE POLICY "alert_delete_admin" ON public.accountability_alerts FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "lesson_select_all_shared_or_own" ON public.lessons_learned;
CREATE POLICY "lesson_select_all_shared_or_own" ON public.lessons_learned FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR author_id = public.current_member_id()
    OR shared_with_all = true
    OR EXISTS (SELECT 1 FROM public.opportunities o WHERE o.id = lessons_learned.related_opportunity_id AND (o.created_by = public.current_member_id() OR o.suggested_by = public.current_member_id()))
    OR EXISTS (SELECT 1 FROM public.tenders t WHERE t.id = lessons_learned.related_tender_id AND (t.assigned_lead = public.current_member_id() OR t.created_by = public.current_member_id()))
  );
DROP POLICY IF EXISTS "lesson_insert_auth" ON public.lessons_learned;
CREATE POLICY "lesson_insert_auth" ON public.lessons_learned FOR INSERT TO authenticated WITH CHECK (true);
...
DROP POLICY IF EXISTS "lesson_delete_admin_or_author" ON public.lessons_learned;
CREATE POLICY "lesson_delete_admin_or_author" ON public.lessons_learned FOR DELETE TO authenticated
  USING (public.is_platform_admin() OR author_id = public.current_member_id());

DROP POLICY IF EXISTS "report_select_admin_or_related" ON public.reports;
CREATE POLICY "report_select_admin_or_related" ON public.reports FOR SELECT TO authenticated
...
CREATE POLICY "report_insert_auth" ON public.reports FOR INSERT TO authenticated WITH CHECK (true);
...
DROP POLICY IF EXISTS "report_delete_admin" ON public.reports;
CREATE POLICY "report_delete_admin" ON public.reports FOR DELETE TO authenticated USING (public.is_platform_admin());

DROP POLICY IF EXISTS "ai_log_select_admin_or_target" ON public.ai_advisor_logs;
CREATE POLICY "ai_log_select_admin_or_target" ON public.ai_advisor_logs FOR SELECT TO authenticated
  USING (
    public.is_platform_admin()
    OR generated_for = public.current_member_id()
    OR EXISTS (SELECT 1 FROM public.tenders t WHERE t.id = ai_advisor_logs.tender_id AND (t.assigned_lead = public.current_member_id() OR t.created_by = public.current_member_id()))
  );
DROP POLICY IF EXISTS "ai_log_insert_auth" ON public.ai_advisor_logs;
CREATE POLICY "ai_log_insert_auth" ON public.ai_advisor_logs FOR INSERT TO authenticated WITH CHECK (true);

DROP POLICY IF EXISTS "ext_platform_read_all" ON public.external_platforms;
CREATE POLICY "ext_platform_read_all" ON public.external_platforms FOR SELECT TO authenticated USING (true);
DROP POLICY IF EXISTS "ext_platform_admin_manage" ON public.external_platforms;
CREATE POLICY "ext_platform_admin_manage" ON public.external_platforms FOR ALL TO authenticated USING (public.is_platform_admin()) WITH CHECK (public.is_platform_admin());